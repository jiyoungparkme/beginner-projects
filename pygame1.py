...
class Python(object):
    ...
    def create(self):
        self.length = 2
        self.positions = [((WINDOW_WIDTH/2), (WINDOW_HEIGHT / 2))] # 뱀길이 때문에 positions
    ...
    def move(self):
        ...
        if new in self.positions[2:]: # 뱀이 스스로의 위치에서 먹어버리면
            self.create() # 새롭게 다시 시작
        else:
...
def show_info(length, speed, surface):
    ...
    text = font.render("Length: " + str(length) + "     Spees: " + str(round(speed, 2)), 1, GL_CONTEXT_PROFILE_COMPATIBILITY)
    pos = text.get_rect()
    pos.centerx = 150
